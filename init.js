...
(function(global, $) {
	...
	codiad.chrome = {
		...
		init: function() {
			...
			amplify.subscribe('context-menu.onShow', function(obj) {
				if ($(obj.e.target).hasClass('directory')) {
					...
					$('#context-menu').append('<a class="directory-only chromeToolkit" onclick="codiad.chrome.buildDownloadZipFile();"><span class="icon-export"></span>Download Chrome Package as ZIP file</a>');
		...
		addChromeManifest: function() {
			...
			var controller = curpath + 'controller.php';
			$.get(controller + '?' + $.param({
				'action': 'addChromeManifest',
				'projectName': codiad.project.getCurrent()
			}), function(data) {
				createResponse = codiad.jsend.parse(data);
				if (createResponse != 'error') {
					codiad.message.success(createResponse.message);
		...
		downloadZIPPath: function() {
			return codiad.project.getCurrent() + '/Build/' + codiad.project.getCurrent() + '.zip';
		},
		downloadCRXPath: function() {
			return codiad.project.getCurrent() + '/Build/' + codiad.project.getCurrent() + '.crx';
		},
		buildDownloadZipFile: function() {
			var _this = this;
			var controller = curpath + 'controller.php';
			$.get(controller + '?' + $.param({
				'action': 'buildDownloadZipFile',
				'projectName': codiad.project.getCurrent()
			}), function(data) {
				createResponse = codiad.jsend.parse(data);
				if (createResponse != 'error') {
					codiad.message.success(createResponse.message);
					codiad.filemanager.rescan(codiad.project.getCurrent());
					codiad.filemanager.download(_this.downloadZIPPath());
				} else {
					codiad.message.error(createResponse);
				}
			});
		...
		buildUserSignedCRX: function() {
			...
			$('#modal-content form').live('submit', function(e) {
				...
				$.get(controller + '?' + $.param({
					'action': 'buildUserSignedSignedCRX',
					'projectName': codiad.project.getCurrent(),
					'pkey': pkey
				}), function(data) {
					createResponse = codiad.jsend.parse(data);
